...
pub fn concatenate_wav_buffers(buffers: &[Vec<u8>], silence_duration_ms: u32) -> Result<Vec<u8>> {
    if buffers.is_empty() {
        return Err(anyhow!(
            "Nenhum buffer de áudio fornecido para concatenação"
        ));
    }

    // Lê as especificações do primeiro buffer para definir a saída
    let first_reader =
        WavReader::new(Cursor::new(&buffers[0])).context("Falha ao abrir primeiro buffer WAV")?;
    ...
    let num_silence_samples =
        (target_spec.sample_rate as u64 * silence_duration_ms as u64 / 1000) as usize;
    ...
    for (idx, buf) in buffers.iter().enumerate() {
        ...
        match (current_spec.sample_format, target_spec.sample_format) {
            ...
            _ => {
                return Err(anyhow!(
                    "Formatos de amostra incompatíveis na concatenação WAV"
                ));
            }
        }
