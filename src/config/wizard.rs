...
pub fn run_wizard() -> Result<()> {
    'wizard_loop: loop {
        println!("\n=== Pheme Setup Wizard / Assistente de Configuração ===\n");

        let language = Select::new(
            "Select episode language / Selecione o idioma dos episódios:",
            vec!["pt-BR", "en-US", "<- Cancel / Voltar ao Menu Principal"],
        )
        .prompt()
        .context("Failed to select language / Falha ao selecionar idioma")?
        .to_string();

        if language.contains("Voltar") || language.contains("Cancel") {
            println!("Wizard canceled / Configuração cancelada.");
            return Ok(());
        }
        ...
        let mode_options = if is_english {
            vec![
                "API KEY (Cloud / Remote API)",
                "LOCAL (Offline / Local)",
                "<- Restart / Recomeçar do início",
            ]
        } else {
            vec![
                "API KEY (Nuvem / Remote API)",
                "LOCAL (Offline / Local)",
                "<- Restart / Recomeçar do início",
            ]
        };
        ...
        if chosen_mode.contains("Recomeçar") || chosen_mode.contains("Restart") {
            continue 'wizard_loop;
        }
        ...
        let (llm_provider, llm_base_url, llm_api_key_env, llm_model) = if is_cloud_mode {
            ...
            if chosen_p.contains("Recomeçar") || chosen_p.contains("Restart") {
                continue 'wizard_loop;
            }
        ...
        let (emb_provider, emb_base_url, emb_api_key_env, emb_model) = if is_cloud_mode {
            ...
            if chosen_e.contains("Recomeçar") || chosen_e.contains("Restart") {
                continue 'wizard_loop;
            }
        ...
        // Final Confirmation
        let confirm_action = Select::new(
            if is_english {
                "Configuration Summary Ready. Select action:"
            } else {
                "Resumo da Configuração Pronto. Selecione a ação:"
            },
            if is_english {
                vec![
                    "1. Save Configuration & Finish",
                    "2. Start Over / Recomeçar Configuração",
                    "3. Cancel & Return to Main Menu",
                ]
            } else {
                vec![
                    "1. Salvar Configuração e Finalizar",
                    "2. Recomeçar Configuração",
        ...
        if confirm_action.starts_with("2") {
            continue 'wizard_loop;
        } else if confirm_action.starts_with("3") {
            println!("Wizard canceled / Configuração cancelada.");
            return Ok(());
        }
